...
-- Library functions 
    -- Misc functions
        ...
        -- Mobile + PC input helpers
        local function is_primary_input(input)
            return input.UserInputType == Enum.UserInputType.MouseButton1
                or input.UserInputType == Enum.UserInputType.Touch
        end

        local function is_move_input(input)
        ...
        function library:get_scale()
            local viewport = camera.ViewportSize
            -- Base design resolution (PC)
            local base_x, base_y = 1920, 1080
            local scale = math.min(viewport.X / base_x, viewport.Y / base_y)

            if library:is_mobile() then
                -- Slightly larger minimum scale on mobile so UI stays usable
                scale = math.clamp(scale * 1.35, 0.55, 1.15)
            else
                scale = math.clamp(scale, 0.65, 1.25)
            end

            return scale
        end

        function library:apply_ui_scale(gui, scale)
            scale = scale or library:get_scale()
            local existing = gui:FindFirstChildOfClass("UIScale")
            if existing then
                existing.Scale = scale
            else
                local ui_scale = Instance.new("UIScale")
                ui_scale.Name = "MobilePCScale"
        ...
        function library:resizify(frame) 
            ...
            -- Larger hitbox for mobile touch
            local hit = library:is_mobile() and 28 or 14
            Frame.Size = dim2(0, hit, 0, hit)
            ...
            local sink_name = "MileniumResizeSink_" .. tostring(frame)

            local function sink_camera()
                if resizing then
            ...
            local function start_resize(input)
                ...
                pcall(function()
                    cas:BindActionAtPriority(sink_name, sink_camera, false, 3000,
            ...
            local function stop_resize()
                ...
                active_input = nil
                pcall(function()
                    cas:UnbindAction(sink_name)
                end)
            end

            Frame.InputBegan:Connect(function(input)
                if is_primary_input(input) and not resizing then
                    start_resize(input)
                end
            end)

            Frame.InputEnded:Connect(function(input)
                if input == active_input or is_primary_input(input) then
                    stop_resize()
                end
            end)
            ...
            library:connection(uis.InputChanged, function(input, game_event) 
                if resizing and is_move_input(input) and (not active_input or input == active_input or input.UserInputType == Enum.UserInputType.MouseMovement) then
                    local viewport_x = camera.ViewportSize.X
                    local viewport_y = camera.ViewportSize.Y
                    local scale = library:get_scale()

                    local current_size = dim2(
                        start_size.X.Scale,
                        math.clamp(
                            start_size.X.Offset + (input.Position.X - start.X) / scale,
                            og_size.X.Offset,
                            viewport_x / scale
                        ),
                        start_size.Y.Scale,
                        math.clamp(
                            start_size.Y.Offset + (input.Position.Y - start.Y) / scale,
                            og_size.Y.Offset,
                            viewport_y / scale
                        )
                    )
        ...
        function library:draggify(frame)
            ...
            local start 
            local active_input = nil
            local sink_name = "MileniumDragSink_" .. tostring(frame)

            local function sink_camera(_, input_state)
                if dragging then
                    return Enum.ContextActionResult.Sink
                end
                return Enum.ContextActionResult.Pass
            ...
            local function start_drag(input)
                ...
                -- Prevent camera rotation / character movement while dragging the UI
                pcall(function()
                    cas:BindActionAtPriority(sink_name, sink_camera, false, 3000,
                        Enum.UserInputType.Touch,
                        Enum.UserInputType.MouseMovement,
            ...
            frame.InputBegan:Connect(function(input)
                if is_primary_input(input) and not dragging then
                    start_drag(input)
                end
            end)
            ...
            library:connection(uis.InputChanged, function(input, game_event) 
                if dragging and is_move_input(input) and (not active_input or input == active_input or input.UserInputType == Enum.UserInputType.MouseMovement) then
                    local viewport_x = camera.ViewportSize.X
                    local viewport_y = camera.ViewportSize.Y
                    local scale = library:get_scale()
                    ...
                    local current_position = dim2(
                        ...
                        clamp(
                            start_size.X.Offset + (input.Position.X - start.X) / scale,
                            0,
                            (viewport_x / scale) - frame.Size.X.Offset
                        ),
                        0,
                        math.clamp(
                            start_size.Y.Offset + (input.Position.Y - start.Y) / scale,
                            0,
                            (viewport_y / scale) - frame.Size.Y.Offset
                        )
                    )
    ...
    -- Library element functions
        function library:window(properties)
            ...
            -- Adaptive default size for mobile
            if not properties.size and not properties.Size and library:is_mobile() then
                local vp = camera.ViewportSize
                local w = math.clamp(math.floor(vp.X * 0.92), 320, 700)
                local h = math.clamp(math.floor(vp.Y * 0.78), 380, 565)
                cfg.size = dim2(0, w, 0, h)
            end
            ...
            local items = cfg.items; do
                items[ "main" ] = library:create( "Frame" , {
                    ...
                    Active = true; -- blocks camera / character movement while interacting with UI
                    Selectable = false;
        ...
        function library:slider(options) 
            ...
            library:connection(uis.InputChanged, function(input)
                if cfg.dragging and is_move_input(input) then 
        ...
        function library:colorpicker(options) 
            ...
            library:connection(uis.InputChanged, function(input)
                if (dragging_sat or dragging_hue or dragging_alpha) and is_move_input(input) then
                    cfg.update_color() 
                end
            end)
